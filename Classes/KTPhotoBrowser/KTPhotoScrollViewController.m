...
#define SHEET_DELETE 100
#define SHEET_EXPORT 101
...
- (void)loadView 
{
   ...
   if ([dataSource_ respondsToSelector:@selector(exportImageAtIndex:actionIndex:)])
   {
...
- (void) exportPhoto
{
    UIActionSheet *sheet = [[UIActionSheet alloc] initWithTitle:@"分享你的创意"   delegate:self
                                              cancelButtonTitle:nil
                                         destructiveButtonTitle:nil
                                              otherButtonTitles:nil];
    sheet.tag = SHEET_EXPORT;

    [sheet addButtonWithTitle:@"分享给微信好友"];
    [sheet addButtonWithTitle:@"分享到朋友圈"];
    [sheet addButtonWithTitle:@"分享给邮件好友"];
    [sheet addButtonWithTitle:@"存到本地相册"];
//    [sheet addButtonWithTitle:@"分享到新浪微博"];
//    [sheet addButtonWithTitle:@"分享到腾讯微博"];
//    [sheet addButtonWithTitle:@"分享到网易微博"];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex 
{
    ...
    else if (actionSheet.tag == SHEET_EXPORT)
    {
        ...
        if ([dataSource_ respondsToSelector:@selector(exportImageAtIndex:actionIndex:)])
            [dataSource_ exportImageAtIndex:currentIndex_ actionIndex:buttonIndex];
