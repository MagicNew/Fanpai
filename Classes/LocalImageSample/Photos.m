...
- (void)exportImageAtIndex:(NSInteger)index actionIndex:(NSInteger)actionIndex
{
   NSString *name = [[self fileNames] objectAtIndex:index];
   NSString *path = [[self photosPath] stringByAppendingPathComponent:name];
   
   if (delegate_  && [delegate_ respondsToSelector:@selector(exportImageAtPath:actionIndex:)]) {
       [delegate_ exportImageAtPath:path actionIndex:actionIndex];
   }   
}
